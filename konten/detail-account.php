          ...
          <?php $keyakses="Detail Account";?>
            ...
            <div class="right-wrapper pull-right">
              <ol class="breadcrumbs">
                ...
                <li><span><?php echo"Account";?></span></li>
                </li>
...
if(isset($_POST['add_contact'])){
	...
	foreach ($_POST['contact_type'] as $type => $value){
		$add_kontak=mysql_query("insert into customer_contact(contact_id,contact_name,contact_type,contact_data,sales,sm) value ('$_GET[detail]','$_POST[contact_name]','$value','$data[$type]','$_SESSION[userid]','$_SESSION[sm]')");
 			...
 			header("location:?hal=detail-account&detail=$_GET[detail]");
 		}
 		else{
...
if(isset($_POST['edit_status'])){
	$reason=$_POST['unreason'];
	$address=getaddress($_POST['lat'],$_POST['lng']);
	if($_POST['unreason']!=null){
		$edit_status=mysql_query("insert into unqualified select * from account where id='$_GET[detail]'");
		if($edit_status){
			mysql_query("update unqualified set status='Unqualified' where id='$_GET[detail]'");
			mysql_query("delete from account where id='$_GET[detail]'");
			mysql_query("insert into history (id,history_action,keterangan,date,time,lokasi,lat,lng,userid) values ('$_GET[detail]','Unqualified','$reason <br/> posted by $_SESSION[nama]','$date','$time','$address','$_POST[lat]','$_POST[lng]','$_SESSION[userid]')");
		...
		$edit_status=mysql_query("update account set status='Working' where id='$_GET[detail]'");
		if($edit_status){
			$_SESSION['sukses']="Success";
 			header("location:?hal=detail-account&detail=$_GET[detail]");
 		}
...
if(isset($_POST['edit_contact'])){
	...
	if($kueri){
		$_SESSION['sukses']="success";
		mysql_query("insert into history (id,history_action,keterangan,date,time,lokasi,lat,lng,userid) values ('$_POST[id_nya]','Edit Contact','Edit contact $_POST[contact_name]','$date','$time','$address','$_POST[lat]','$_POST[lng]','$_SESSION[userid]')");
	}
	else{
		$_SESSION['error']="error";
	}
...
if(isset($_POST['delete_contact'])){
	...
	if($kueri){
		$_SESSION['sukses']="success";
		mysql_query("insert into history (id,history_action,keterangan,date,time,lokasi,lat,lng,userid) values ('$_POST[kontakid]','Delete Contact','Delete contact $_POST[kontak]','$date','$time','$address','$_POST[lat]','$_POST[lng]','$_SESSION[userid]')");
	}
	else{
		$_SESSION['error']="error";
	}
	header("location:?hal=detail-account&detail=$_GET[detail]");
		...
		if(isset($_GET['detail'])){
			...
			elseif($_SESSION['level']=="1"){
				$cek=mysql_num_rows(mysql_query("select * from account where id='$_GET[detail]' and sm='$_SESSION[userid]'"));
			}
			elseif($_SESSION['level']=="2"){
				$cek=mysql_num_rows(mysql_query("select * from account where id='$_GET[detail]' and sales='$_SESSION[userid]'"));
			}
		if($cek == "0"){
...
<?php }?>
	...
	<div class="panel-body">
        	...
        	$address=getaddress($_POST['lat'],$_POST['lng']);
 			...
 			if($edit_opportunity){
 			mysql_query("insert into history (id,history_action,keterangan,date,time,lokasi,lat,lng,userid) values ('$_GET[detail]','Edit Oppurtunity','edit opportunity $package','$date','$time','$address','$_POST[lat]','$_POST[lng]','$_SESSION[userid]')");
