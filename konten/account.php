...
<?php
  if ($_SESSION['level']=="1"){
                        $kueri=mysql_query("select * from account where sm='$_SESSION[userid]'");
                        $jumlahkueri=mysql_num_rows($kueri);
                    }
                    elseif($_SESSION['level']=="2"){
                        $kueri=mysql_query("select * from account where sales='$_SESSION[userid]'");
                        $jumlahkueri=mysql_num_rows($kueri);
                    }
                    elseif($_SESSION['level']=="0"){
                        $kueri=mysql_query("select * from account");
                        $jumlahkueri=mysql_num_rows($kueri);
                    }?>
...
if($_SESSION['level']=="0" or $_SESSION['level']=="1"){echo'<div class="col-lg-12" style="padding-bottom: 20px;">';}else{echo'<div class="col-lg-12" style="padding-bottom: 20px;">';}?>
        ...
        <section class="panel">
                    ...
                    <tbody>
                     <?php while ($kolomlist = mysql_fetch_assoc($kueri)) {
                        ...
                        echo' <tr class="gradeX">
                           <td data-title="Name"><a href=?hal=detail-account&detail='.$kolomlist['id'].'>'.$kolomlist['nama'].' '.$kolomlist['lname'].'</a></td>
            ...
            <div class='card'>
            <input type="search" class="form-control" id="input-search" placeholder="Cari berdasarkan nama account, nama institusi, nama customer, dsb ...." >
                ...
                <div class=\"row\">
                    ...
                    <div class=\"col-xs-9 col-md-9 section-box\">
                        <h2><font color=\"#31B0D5\">
                            <a href=?hal=detail-account&detail=$kolom[id]>".substr($kolom["nama"]." ".$kolom['lname'],0,17). " </a>
                            </span>
                        </h2></font>
                        <table style=\"100%\">
                        <tr>
...
echo "<center><h1>You don't have any account</center>";}?>
        </div>
    </div>
</div>
</footer>
