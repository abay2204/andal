...
if($hitungactivity>0){
  while($pecahactivity=mysql_fetch_array($activity)){
    $leads=mysql_query("select * from leads where id='$pecahactivity[id]'");
    $leadshitung=mysql_num_rows($leads);
    if($leadshitung>0){
      $namaleads=mysql_fetch_array($leads);
      $namanya=$namaleads['nama'];
    }
    else{
      $namaakun=mysql_fetch_array(mysql_query("select * from account where id='$pecahactivity[id]'"));
      $namanya=$namaakun['nama'];
    }
      $aktifitas=$pecahactivity['keterangan'];
          echo'
                    <div class="panel-body" data-toggle="tooltip" title="'.$pecahactivity['lokasi'].'" data-placement="top"">
                      ...
                      '.$pecahactivity['history_action'].' '.$namanya.'<br/>
                      '.$aktifitas.'
        </div>
                 ';
  }}
else{
  echo "Tidak ada aktifitas";
}
  echo"</div>";
  $i++;
  echo'</div></div>';
